...
### 4. 验证 Everforest Dark 主题
1. 登录 Memos，点击左侧导航栏的 **Settings**（设置）按钮，选择 **Preferences**（偏好设置）。
2. 在 **Theme**（主题）选择下拉框中，确认存在一个带有 `Leaf`（叶子）图标的 **Everforest Dark** 选项。
3. 选择该主题，验证页面背景瞬间切换为护眼舒服的石板绿暗色系（Everforest 经典色 `#2b3339`），且文字、边框与高亮均完美契合 Everforest 调色盘。
